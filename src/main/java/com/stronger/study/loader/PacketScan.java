...
public abstract class PacketScan {
    ...
    private HashMap<String,Class<?>> packetScanner(File curFile, String packName,HashMap<String,Class<?>> classes) {
        //如果不是目录就结束方法的调用
        ...
        for(File file : files) {
            if (file.isFile() && file.getName().endsWith(".class")) {
                ...
                //一直这样递归调用，直到是文件为止
                packetScanner(file, packName + "." + file.getName(),classes);
            }
        }
        return classes;
    }

    private HashMap<String,Class<?>> scanJarPacket(URL url,HashMap<String,Class<?>> classes) throws IOException {
        /**
         * 由API查得：
    ...
    //对包扫描方法重载，可以扫描多个包
    public void scanPacket(String[] packetNamees,HashMap<String,Class<?>> classes) {
        for(String packetName : packetNamees) {
            scanPacket(packetName,classes);
        }
    }
    //对包扫描方法重载，提供多个类名，可以扫描该类所在的包
    public void scanPacket(Class<?>[] klasses,HashMap<String,Class<?>> classes) {
        for(Class<?> klass : klasses) {
            scanPacket(klass,classes);
        }
    }

    public void scanPacket(Class<?> klass,HashMap<String,Class<?>> classes) {
        String path = klass.getPackage().getName();
        scanPacket(path,classes);
    }

    public HashMap<String,Class<?>> scanPacket(String packetName,HashMap<String,Class<?>> classes) {
        /**
         * 由API查得：
        ...
        try {
            ...
            while(resources.hasMoreElements()) {
                ...
                if (url.getProtocol().equals("jar")) {
                    ...
                    classes = scanJarPacket(url,classes);
                } else {
                    ...
                    //如果是普通包就调用普通包扫描的方法
                    classes = packetScanner(file, packetName,classes);
                }
            }
